...
import like.like_main as like_main
...
# 계정 읽기
accounts = common.get_accounts('like_account.txt')
...
def fetch_order() -> bool:
    ...
    res = requests.get(config['api']['url'],
                        data={
                            'key': config['api']['key'],
                            'action': 'getOrder',
                            'type': config['api']['like_service']
                        }, timeout=10).json()
    # 결과가 성공이 아니면
    if res['status'] != 'success':
        order_id = config['item']['test_like_order_id']
        quantity = int(config['item']['test_like_quantity'])
        order_url = str(config['item']['test_like_order_url'])
        mode = "TEST"
    else:
        order_id = res['id']
